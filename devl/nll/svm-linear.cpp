...
# pragma warning( push )
# pragma warning( disable:4127 4996 4701 ) // condition expression constant, unsafe
...
namespace algorithm
{
   ...
   public:
	   ...
	   void tron(double *w)
      {
         // Parameters for updating the iterates.
	      ...
	      while (iter <= max_iter && search)
	      {
		      ...
		      if (actred < eta0*prered)
               delta = std::min<double>(std::max(alpha, sigma1)*snorm, sigma2*delta);
	   ...
	   void set_print_string(void (*print_string) (const char *buf))
      {
