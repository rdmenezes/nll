...
namespace algorithm
{
   ...
   //
   void Fft1D::forward( const core::Buffer1D<double>& real, size_t nfft, core::Buffer1D<double>& outComplex )
   {
      ...
      const size_t nc = ( nfft / 2 ) + 1;
      fftw_complex* out = reinterpret_cast<fftw_complex*>( fftw_malloc( sizeof ( fftw_complex ) * nc ) );
      fftw_plan plan_forward = fftw_plan_dft_r2c_1d( static_cast<int>( nfft ), in, out, FFTW_ESTIMATE );
   ...
   void Fft1D::backward( const core::Buffer1D<double>& complex, size_t nfft, core::Buffer1D<double>& outReal )
   {
      ...
      fftw_plan plan_backward = fftw_plan_dft_c2r_1d( static_cast<int>( sizeOut ), in, out, FFTW_ESTIMATE );
...
namespace algorithm
{
   ...
   //
   void Fft1D::forward( const core::Buffer1D<double>& real, size_t nfft, core::Buffer1D<double>& )
   {
      throw std::runtime_error( "No FFT library binded" );
   }

   void Fft1D::backward( const core::Buffer1D<double>& real, size_t nfft, core::Buffer1D<double>& )
   {
      throw std::runtime_error( "No FFT library binded" );
   }
}
