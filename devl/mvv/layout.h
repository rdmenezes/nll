...
namespace mvv
{
   ...
   /**
    Base class that holds a list of pane to display. draw() and updateLayout() method must be implemented
    */
   ...
   public:
      ...
      virtual void updateLayout()
      {
         ...
         for ( Panes::iterator it = _panes.begin(); it != _panes.end(); ++it, ++n )
            ...
            {
               Pane* p = *it;
               ratioShift += _ratios[ n ];
               double ratio = _ratios[ n ] / ratioToDraw;
               p->setOrigin( nll::core::vector2ui( getOrigin()[ 0 ] + (ui32)nll::core::round( n * ratioShift * getSize()[ 0 ] ), getOrigin()[ 1 ] ) );
