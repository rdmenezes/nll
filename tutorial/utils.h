...
namespace utility
{
   ...
   template <class Database>
   core::Image<ui8> printProjectionLLE( ui32 sizex, ui32 sizey, const Database& dat )
   {
      sizex /= 2;
      sizey /= 2;
      ...
      typedef typename Database::Sample::Input  Input;
      typedef nll::core::DatabaseInputAdapterRead<Database> Adapter;
      typedef nll::algorithm::LocallyLinearEmbedding Projection;

      Adapter points( dat );
      Projection p;
      std::vector<Input> ppoints = p.transform( points, 2, 12 );
